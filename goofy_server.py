...
class GoofyServer:
            ...
            while self._running:
                ...
                if isinstance(packet, GoofyCommandSetLimits):
                    ...
                    t = threading.Thread(
                        ...
                        name=f"[open {packet.dst_addr}:{packet.dst_port}] "
    ...
    def _cmd_open_socket(self, packet: GoofyCommandOpenSocket):
        try:
            ...
            bind_addr, bind_port = target.getsockname()[:2]
            ...
            sock.lock.release()
            self.log.debug(
                f"relay planned: {packet.dst_addr}:{packet.dst_port} "
                f"<-> goofy client"
            )
        except BaseException as e:
    ...
    def _cmd_bind(self, packet: GoofyCommandBind):
        try:
            ...
            try:
                ...
                bind_sock.listen(1)
                bind_addr, bind_port = bind_sock.getsockname()
            except OSError as e:
            ...
            self.log.debug(f"listening on {bind_addr}:{bind_port}")

            # wait for the expected remote peer to connect
            try:
                remote_sock, remote_full_addr = bind_sock.accept()
                remote_sock.settimeout(self.timeout)
                remote_addr = remote_full_addr[0]
                remote_port = remote_full_addr[1]
            except OSError as e:
                close_socket(bind_sock)
            ...
            self.log.debug(
                f"inbound connection from {remote_addr}:{remote_port}"
            )
            ...
            sock.lock.release()
            self.log.debug(
                f"relay planned: {remote_addr}:{remote_port} <-> goofy client"
            )
        except BaseException as e:
            self.log.warning(format_exception(e))
