...
from ghio import GhIo, MAX_FILE_AGE
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="goofy proxy on ghio: share your internet connection with "
        "a friend by creating and reading files on a GitHub repository."
    )
    ...
    parser_server = subparsers.add_parser(
        "server",
        help="run GoofyServer on top of ghio."
    )
    parser_client = subparsers.add_parser(
        "client",
        help="run GoofyClient on top of ghio. the peer must be running "
    ...
    for p in (parser_server, parser_client):
        p.add_argument(
            "token",
            type=str,
            help="GitHub Personal Access Token. avoid using tokens with full "
            "access permissions. use fine-grained tokens with access to only "
            "the repository used for data transfer with only the permission to "
            "read and modify its content."
        )
        p.add_argument(
            "repo_owner",
            type=str,
